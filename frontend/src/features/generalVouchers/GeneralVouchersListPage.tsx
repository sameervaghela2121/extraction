...
import { generalVouchersApi, type GeneralVoucherQuery } from "../../api/generalVouchers.api";
...
export default function GeneralVouchersListPage() {
  ...
  // Page lives in the URL (not just component state) so that navigating into a voucher
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const query: GeneralVoucherQuery = { search, sort, order, page, pageSize: 10 };
      setData(await generalVouchersApi.list(query));
    } catch (err) {
  ...
  const bulk = async (action: "verify" | "unverify" | "archive") => {
    const ids = [...selected];
    try {
      if (action === "verify") await generalVouchersApi.bulkVerify(ids);
      if (action === "unverify") await generalVouchersApi.bulkUnverify(ids);
      if (action === "archive") await generalVouchersApi.bulkArchive(ids);
      const verb = action === "archive" ? "deleted" : action === "unverify" ? "marked as pending" : "verified";
      notify(`${ids.length} voucher${ids.length > 1 ? "s" : ""} ${verb}`);
      setSelected(new Set());
      load();
    } catch (err) {
      notify(apiErrorMessage(err), "error");
  ...
  return (
    <div>
      <PageHeader title="General Vouchers" subtitle="Review and verify extracted vouchers." />
          ...
          <table className="table">
              ...
              <tr>
                ...
                <th style={{ cursor: "pointer" }} onClick={() => toggleSort("title")}>
                  <span className="row gap-8" style={{ display: "inline-flex" }}>
                    Voucher {sort === "title" && (order === "asc" ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}
                  </span>
                </th>
            ...
            <tbody>
              {items.map((d: DocumentListItem) => (
                <tr key={d.id} style={{ cursor: "pointer" }} onClick={() => navigate(`/general-vouchers/${d.id}`)}>
                  <td onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" checked={selected.has(d.id)} onChange={() => toggleOne(d.id)} />
                  </td>
                  <td>
                    <div style={{ fontWeight: 600 }}>{d.title}</div>
                    <div className="faint" style={{ fontSize: 12 }}>Voucher · via {d.source}</div>
                  </td>
